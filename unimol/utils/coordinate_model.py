...
def single_SF_loss(predict_coords, pocket_coords, distance_predict, holo_distance_predict, dist_threshold=4.5):
    dist = torch.norm(predict_coords.unsqueeze(1) - pocket_coords.unsqueeze(0), dim=-1)
    holo_dist = torch.norm(predict_coords.unsqueeze(1) - predict_coords.unsqueeze(0), dim=-1)
    distance_mask = distance_predict < dist_threshold
    cross_dist_score = ((dist[distance_mask] - distance_predict[distance_mask])**2).mean()
    dist_score = ((holo_dist - holo_distance_predict)**2).mean()
    loss = cross_dist_score*1.0 + dist_score*5.0
    return loss


def scoring(predict_coords, pocket_coords, distance_predict, holo_distance_predict, dist_threshold=4.5):
...
def dock_with_gradient(coords, pocket_coords, distance_predict_tta, holo_distance_predict_tta, loss_func=single_SF_loss, holo_coords=None, iterations=20000, early_stoping=5):
    bst_loss, bst_coords, bst_meta_info = 10000.0, coords, None
    for i, (distance_predict, holo_distance_predict) in enumerate(zip(distance_predict_tta, holo_distance_predict_tta)):
        new_coords = copy.deepcopy(coords)
        _coords, _loss, _meta_info = single_dock_with_gradient(new_coords, pocket_coords, distance_predict, holo_distance_predict, loss_func=loss_func, holo_coords=holo_coords, iterations=iterations, early_stoping=early_stoping)
        if bst_loss > _loss:
            bst_coords = _coords
            bst_loss = _loss
...
def single_dock_with_gradient(coords, pocket_coords, distance_predict, holo_distance_predict, loss_func=single_SF_loss, holo_coords=None, iterations=20000, early_stoping=5):
...
def add_coord(mol, xyz):
    ...
    for i in range(pos.shape[0]):
        conf.SetAtomPosition(i, Chem.rdGeometry.Point3D(pos[i][0], pos[i][1], pos[i][2]))
    return mol


def single_docking(input_path, output_path, output_ligand_path):
    content = pd.read_pickle(input_path)
    init_coords_tta, mol, smi, pocket, pocket_coords, distance_predict_tta, holo_distance_predict_tta, holo_coords, holo_cener_coords = content
    ...
    for i in range(sample_times):
        ...
        predict_coords, loss, meta_info = dock_with_gradient(init_coords, pocket_coords, distance_predict_tta, holo_distance_predict_tta, holo_coords=holo_coords, loss_func=single_SF_loss)
        if loss < bst_loss:
            bst_loss = loss
    ...
    if output_path is not None:
        with open(output_path, 'wb') as f:
            pickle.dump([bst_predict_coords, holo_coords, bst_loss, smi, pocket, pocket_coords], f)
