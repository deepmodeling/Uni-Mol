...
class MolConfGLoss(UnicoreLoss):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        distance_loss, coord_loss = self.compute_loss(model, net_output, sample, reduce=reduce)
        sample_size = sample["target"]["coord_target"].size(0)
        loss = self.args.coord_loss * coord_loss + self.args.distance_loss * distance_loss
        logging_output = {
            "loss": loss.data,
...
def realign_coord(coord_predict, coord_target, token_mask):
    ...
    for i in range(bs):
        ...
        _r = R.align_vectors(_coord_target, _coord_predict)[0].as_matrix().astype(np.float32)
        _new_coord_target = torch.from_numpy(np.dot(_coord_target, _r)).type_as(coord_target)
...
class MolConfGInferLoss(UnicoreLoss):
    def __init__(self, task):
        super().__init__(task)
    ...
    def forward(self, model, sample, reduce=True):
        ...
        logging_output = {
            ...
            "bsz": sample["target"]["coord_target"].size(0),
            "sample_size": 1,
            "coord_rdkit": sample["net_input"]["src_coord"].data,
    ...
    def compute_loss(self, model, net_output, sample, reduce=True):
        ...
        # distance loss
        distance_predict = torch.norm(coord_predict.unsqueeze(2) - sample["target"]["coord_target"].unsqueeze(1), dim=-1)
        distance_predict = distance_predict[distance_mask]
        distance_target = sample["target"]["distance_target"][distance_mask]
