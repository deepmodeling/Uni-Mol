    ...
    def reduce_metrics(logging_outputs, split='valid') -> None:
        ...
        if 'valid' in split or 'test' in split:
            ...
            if predicts.size(-1) == 1:
                ...
                df = pd.DataFrame({'predict': predicts.view(-1).cpu(), "target": targets.view(-1).cpu(), "smi": smi_list})
                ...
                metrics.log_scalar(
                    f'{split}_mae', mae, sample_size, round=3
                )
                metrics.log_scalar(
                    f'{split}_mse', mse, sample_size, round=3
                )
                metrics.log_scalar(
                    f'{split}_agg_mae', agg_mae, sample_size, round=3
                )
                metrics.log_scalar(
                    f'{split}_agg_mse', agg_mse, sample_size, round=3
                )
                metrics.log_scalar(
...
class FinetuneSmoothMAELoss(FinetuneMSELoss):
    ...
    def reduce_metrics(logging_outputs, split='valid') -> None:
        ...
        if 'valid' in split or 'test' in split:
            ...
            y_pred = torch.cat([log.get("predict") for log in logging_outputs], dim=0).view(-1, conf_size, num_task).cpu().numpy().mean(axis=1)
